...
- Add per-project provider pinning: `ccs pin <name>` writes the managed env into `./.claude/settings.local.json` and `ccs unpin` removes the pin while preserving everything else in the file (`ccs use <name> --project` / `ccs use --global` stay as hidden aliases). Secrets are only written when the file is git-ignored; managed keys defined globally but not by the pinned provider are blanked to `""` so they cannot bleed through Claude Code's per-key settings merge (this also covers `ccs pin official`). `ccs current` becomes project-aware with a two-line view that resolves the pin back to a provider name, and `ccs doctor` reports bleed-through keys, missing gitignore coverage, and unpinned project files.
